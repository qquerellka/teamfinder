...
# КРАТКО: авторизация через Telegram Mini App + dev-login.
# =============================================================================
...
class DevLoginIn(BaseModel):
    ...
    Это «фейковый» профиль, который маппится в upsert_from_tg,
...
async def auth_telegram(payload: TelegramInitIn):
    """
    Боевое логин/авторизация через Telegram Mini App (init_data).
    """
    try:
...
async def auth_dev_login(payload: DevLoginIn):
    ...
    В prod-окружении (APP_ENV=prod) отключён.
    """
