# backend/infrastructure/admin_auth.py
...
def require_admin_token(x_admin_token: str | None = Header(default=None, alias="X-Admin-Token")) -> AdminPrincipal:
    """
    Проверяем заголовок X-Admin-Token.
    Если токен не задан или не совпадает с settings.ADMIN_API_TOKEN — кидаем 401.
    """
    if not settings.ADMIN_API_TOKEN:
        # Неправильная конфигурация бэкенда
        raise HTTPException(
