...
export type SIconProps = {
  ...
  /**
   * Цвет иконки.
   * Если не задан — наследуется от родителя (через currentColor),
   * что позволяет автоматически подстраиваться под тему.
   */
  color?: string;
  clickable?: boolean;
  asButton?: boolean;
...
const Wrap = styled.span<{
  ...
  /* если $color не передан — используем currentColor (наследуется от родителя) */
